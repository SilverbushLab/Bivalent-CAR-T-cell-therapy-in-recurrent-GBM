...
srt_merge <- readRDS("~/processed_data/tumors_fig_3_processed_with_umap_reduction.rds") # only do if necessary

srt_merge$sample_name %>% table()
```

# subset to all myeloid and subcluster
There are an uneven number of T cells pre and post infusion. 
randomly subset pre infusion T cells to make a fair comparison in cell abundance (expressing genes of interest) for pre and post infusion
```{r}
...
p2 <- ggplot(df_long, aes(x = Timepoint, y = expression, fill = Timepoint)) +
  ...
  stat_compare_means(method = "wilcox.test", label = "p.signif", position = position_nudge(y = -0.5)) +  # Add Wilcoxon test
