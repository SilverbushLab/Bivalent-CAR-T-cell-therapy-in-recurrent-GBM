...
create_UMAP <- function(seurat_obj, sample_name, num_pcs, res = 0.25, umap_dir = "/path/results/dimreduction/umap/") {
...
```{r}
system.time(
  srt_merge <- readRDS("/path/to/tumors_normalized_merged_scaled_srt_revision.rds")
)
```
...
srt_merge <- create_UMAP(srt_merge, "merged_filtered_processed_revision", res = 0.25, num_pcs =  10)
```

```{r}
p1 <- DimPlot(srt_merge, reduction = "umap", group.by = "seurat_clusters", label = TRUE)

ggsave(filename = "UMAP_merged_filtered_processed_revision_seurat_clusters.jpg",
...
```{r}
saveRDS(srt_merge, "/path/processed_data/tumors_normalized_merged_scaled_dimreduction_srt_revision.rds")
```
