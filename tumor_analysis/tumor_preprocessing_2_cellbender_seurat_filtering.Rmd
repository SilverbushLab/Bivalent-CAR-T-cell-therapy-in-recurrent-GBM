...
```{r}
# we are omitting p16 for this manuscript
sample_names <- c("Tumor_P1_Pre",
                  "Tumor_P1_Post",
                  "Tumor_P4_Pre",
                  "Tumor_P4_Post",
                  "Tumor_P7_Pre",
...
```{r}
srt_obj <- seurat_objects_cellbender$P4_Pre_10973

counts <- srt_obj$nCount_RNA
upper_bound <- quantile(counts, 0.99)
plot_obj <- subset(srt_obj, subset = nCount_RNA < upper_bound)
sample_name <- "P4_Pre_10973"
  # --- Pre-filter Violin Plot ---
# Generate list of plots — one per feature
vln_list <- lapply(c("nFeature_RNA", "nCount_RNA"), function(f) {
  VlnPlot(plot_obj, features = f, pt.size = 0.3) +
    scale_y_continuous(breaks = pretty_breaks(n = 7)) +
    theme(legend.position = "none") +
    ggtitle(f)
})
...
# Combine with patchwork
pre_vln <- wrap_plots(vln_list, ncol = 2) +
  plot_annotation(title = paste0("Pre-filter: ", sample_name))
...
saveRDS(srt_merge, file = paste0(processed_data_dir, "tumors_normalized_merged_srt.rds"))
```
...
top10 <- head(VariableFeatures(srt_merge), 10)

# Plot variable features (with and without labels)
p1 <- VariableFeaturePlot(srt_merge)
p2 <- LabelPoints(plot = p1, points = top10, repel = TRUE, max.overlaps = 100)
...
saveRDS(srt_merge, file = paste0(processed_data_dir, "tumors_normalized_merged_scaled_srt.rds"))
